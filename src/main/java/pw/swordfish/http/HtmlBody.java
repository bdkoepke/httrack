...
public class HtmlBody implements Iterable<HtmlLink>, Iterator<HtmlLink> {
    private static final Pattern htmlTagPattern = Pattern.compile("<a\\b[^>]*href=\"[^>]*>(.*?)", Pattern.CASE_INSENSITIVE);
    private static final Pattern htmlPattern = Pattern.compile("<html.*");
    private final LinkedList<HtmlLink> htmlLinkLinkedList = new LinkedList<HtmlLink>();
    private final Iterator<HtmlLink> htmlLinkLinkedListIterator;
    private String html = "";
    ...
    public HtmlBody(String htmlBody) {
        ...
        while (htmlTagMatcher.find())
            htmlLinkLinkedList.add(new HtmlLink(htmlTagMatcher.group()));
